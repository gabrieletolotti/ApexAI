...
const Header = () => {
  ...
  return (
    <header
      ref={ref}
      className={`fixed left-1/2 -translate-x-1/2 mt-6 w-[95vw] max-w-4xl bg-white/10 backdrop-blur-xl z-50 border border-white/20 shadow-lg flex items-center transition-all duration-300 ease-out ${isIntersecting ? 'translate-y-0 opacity-100' : 'translate-y-0 opacity-100'} ${isMenuOpen ? 'rounded-3xl' : 'rounded-full'}`}
      style={{ top: undefined }}
    >
