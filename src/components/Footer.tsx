        ...
        <div className="flex flex-row flex-wrap gap-12 mb-12 items-start justify-between">
          ...
          <div className={`flex flex-row flex-1 min-w-[400px] space-x-12 animate-on-scroll fade-in-up ${isIntersecting ? 'visible' : ''}`}> 
            ...
            <div className="space-y-2">
              ...
              <Link to="/" className="block hover:text-blue-400 transition-colors duration-300">Home</Link>
              <button onClick={() => { window.scrollTo({ top: 0, behavior: 'instant' }); const heroSection = document.getElementById('services'); if (heroSection) { setTimeout(() => { heroSection.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100); } }} className="block text-left hover:text-blue-400 transition-colors duration-300">Servizi</button>
              <button onClick={() => { window.scrollTo({ top: 0, behavior: 'instant' }); const aboutSection = document.getElementById('about'); if (aboutSection) { setTimeout(() => { aboutSection.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100); } }} className="block text-left hover:text-blue-400 transition-colors duration-300">Chi Siamo</button>
              <Link to="/prenota" className="block hover:text-blue-400 transition-colors duration-300">Contatti</Link>
            </div>
          </div>
